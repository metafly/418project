/*
 *
 * Written and documented by Shivani prasad
 */
 ...
 */
void *MESIF::response_worker(void *arg) {
    MESIF *obj = (MESIF *) arg;
...
/**
 * Implemention for the request worker thread for each cache.
 */
void *MESIF::request_worker(void *arg) {
    MESIF *obj = (MESIF *)arg;
    std::string op;
    unsigned long addr;
 ...
 */
void MESIF::handle_request(MESIF *obj, std::string op, unsigned long addr) {
    assert(addr);
    bool done = false;
    ...
    if ((status == Exclusive && op == "R") || status == Modified ||
        (status == Shared && op == "R") || (status == Forward && op == "R")) {
                ...
                if(op == "R") {
                    ...
                    if(Bus::recv_nak) {
                        ...
                        if(Bus::read_ex == true) {
                            ...
                            obj->cache.insert_cache(addr, Forward);
                        }
